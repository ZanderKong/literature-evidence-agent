...
| # | Hard Gate | 结果 | 证据 |
|---|-----------|------|------|
| 1 | 真实 Provider 无法产生 claims | **❌ FAIL** | `ExtractionResponse(claims=[])` — claims 永远为空 (F018) |
...
| 6 | FTS 查询存在只读写入错误并静默吞错 | **❌ FAIL** | `_sync_claim_fts` 在 read_only 连接 INSERT + `except:pass` (F019) |
...
**Hard Gate 通过率：1/12 → 全部 12 项均失败或未验证**
...
| F008 | 执行日志虚高 | 虚假完成报告 |
| F018 | DeepSeek 响应未解析为 claims | 真实 API 无法产生 claims |
| F019 | FTS 只读写入 + 静默吞错 | 搜索不可靠 |
